...
export default function ProjectDetails(props) {
  ...
  const [brochureLoader, setBrochureLoader] = useState(false);
  ...
  const onSubmit = async () => {
    if (
      ...
      try {
        ...
        formData.append('project_id', props?.route?.params?.id);
        formData.append(
          'project_location',
        ...
        if (err?.response?.status >= 500 && err?.response?.status <= 599) {
          setApiResponseMessageView(
            'Unable to download broucher at the moment.',
          );
      ...
      let emailError = emailRegex(email) == true ? false : true;
      let phoneError =
        phone?.length >= 7 == true && phone?.length <= 15 == true;
      let nameError = fName?.length > 0 ? false : true;
      setErrorsFname(nameError);
      setErrorsPhone(!phoneError);
      if (errorsInvalidEmail == true) {
        setErrorsEmail(false);
      } else {
    ...
    //   setErrorsEmail(true);
    //   setErrorsPhone(true);
    // } else if (fName?.length > 0 && emailRegex(email) == false && phone?.length <= 0) {
    //   setErrorsFname(false);
    //   setErrorsEmail(true);
    //   setErrorsPhone(true);
    // } else if (fName?.length > 0 && emailRegex(email) == true && phone?.length <= 0) {
    //   setErrorsFname(false);
    //   setErrorsEmail(false);
    //   setErrorsPhone(true);
    // } else if (fName?.length <= 0 && emailRegex(email) == true && phone?.length <= 0) {
  ...
  const getBrouchureUrl = async id => {
    try {
      ...
      if (err?.response?.status >= 500 && err?.response?.status <= 500) {
        setApiResponseMessageView('Unable to download broucher at the moment.');
      }
      setBrochureUrl('');
        ...
        <>
          ...
          <TabsButton
            ...
            enquiryPress={() => {
              ...
              // console.log('sjasbdjkasjkdhasjkdh', projectDetails);
              // return;
            }}
          />
          <MoratgeCalculator
