...
import {EventLogsValidation} from '@/components/Validations/validations';
...
const EventLogs = props => {
  ...
  const {
    ...
    resolver: yupResolver(EventLogsValidation),
  });
  useEffect(() => {
  ...
  const onSubmit = async () => {
    ...
    try {
      var payLoad = {
        ...
        location: location,
        accessLevel: directionLable == '' ? null : directionLable?.id,
      ...
      if (error?.response?.status == 401) {
        ...
        setApiResponseMessageView('Unable to create event logs at the moment.');
      } else {
        ...
        <KeyboardAwareScrollView
          ...
          <Input
            ...
            inputViewStyles={{
              borderWidth: 1,
              borderColor:
                focused == 'location' ? theme?.logoColor : theme?.inputBorder,
              height: 46,
              borderRadius: 8,
            }}
            textInputFocused={() => {
            ...
            fieldName={'location'}
            control={control}
            errTxt={errors?.location && errors?.location?.message}
            errTxtstyle={{
              top: 46,
              right: 0,
              position: 'absolute',
          ...
          <View style={{zIndex: 1000, marginTop: 10}}>
            ...
            <DropDownButtonYup
              ...
              label={
                directionLable == '' ? 'Show as busy' : directionLable?.title
              }
              labelStyle={{
          ...
          <Input
            ...
            placeholder={'Start typing to leave a call logs....'}
            fieldName={'desc'}
            control={control}
